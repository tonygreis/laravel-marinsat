<template>
    <main class="mt-2">
        <div
            class="bg-white py-4 px-4 sm:px-6 lg:col-span-3 xl:pl-12 shadow overflow-hidden border-b border-gray-200 rounded-lg">
            <div class="max-w-lg mx-auto lg:max-w-none">
                <form class="flex">
                    <div class="p-1 flex items-center">
                        <label for="tmdb_id_g" class="block text-sm font-medium text-gray-700 md:mr-4">Tmdb Id</label>
                    ...
                    <div class="p-1">
                        <span class="inline-flex rounded-md shadow-sm">
                            <button
                                ...
                                <svg v-if="isGLoading" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white"
                            ...
                            <button @click.prevent="clear"
                                    class="p-2 bg-red-300 hover:bg-red-500 text-white ml-3 rounded-lg">Cancel</button>
                        </span>
                    </div>
                </form>
            </div>
        ...
        <div v-if="isShowingForm"
             class="bg-white py-4 px-4 sm:px-6 mt-2 lg:col-span-3 xl:pl-12 shadow overflow-hidden border-b border-gray-200 rounded-lg">
            <div class="max-w-lg mx-auto lg:max-w-none">
                <form class="grid grid-cols-1 row-gap-6 mt-2">
                    <div class="mt-2 md:flex md:items-center">
                        <label for="tmdb_id" class="block text-sm font-medium text-gray-700 md:mr-4">Tmdb Id</label>
                        <div class="relative rounded-md shadow-sm flex-auto">
                            <input
                                v-model="serie.tmdb_id"
                                id="tmdb_id"
                                name="tmdb_id"
                    ...
                    <div class="mt-2 md:flex md:items-center">
                        <label for="name" class="block text-sm font-medium text-gray-700 md:mr-4">Name</label>
                        <div class="relative rounded-md shadow-sm flex-auto">
                            <input
                                v-model="serie.name"
                                id="name"
                    ...
                    <div class="mt-2 md:flex md:items-center">
                        <label for="created_year" class="block text-sm font-medium text-gray-700 md:mr-4">Created
                            Year</label>
                        <div class="relative rounded-md shadow-sm flex-auto">
                            <input
                                v-model="serie.created_year"
                                id="created_year"
                                name="created_year"
                                class="block w-full transition duration-150 ease-in-out appearance-none bg-white dark:bg-gray-600 dark:text-gray-50 border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5"
                                placeholder="Name">
                    ...
                    <div class="mt-2 md:flex md:items-center">
                        <label for="poster_path" class="block text-sm font-medium text-gray-700 md:mr-4">Poster</label>
                        <div class="relative rounded-md shadow-sm flex-auto">
                            <input
                                v-model="serie.poster_path"
                                id="poster_path"
                                name="poster_path"
                                class="block w-full transition duration-150 ease-in-out appearance-none bg-white dark:bg-gray-600 dark:text-gray-50 border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5"
                                placeholder="Poster">
                        ...
                        <span class="inline-flex rounded-md shadow-sm">
                            <button
                             ...
                             <button
                                 type="submit"
                                 v-if="isEdit"
                                 @click.prevent="updateSerie(serie)"
                                 class="inline-flex items-center justify-center py-2 px-4 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-green-600 hover:bg-green-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-green-700 transition duration-150 ease-in-out disabled:opacity-50">
                                <svg v-if="isLoading" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white"
                                     xmlns="http://www.w3.org/2000/svg" fill="none"
                                     viewBox="0 0 24 24">
                ...
                <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg mt-4">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr v-for="serie in series" :key="serie.id">
                                ...
                                <td class="w-4/12 px-6 py-4 whitespace-no-wrap">
                                    <div class="text-sm leading-5 text-gray-900">{{ serie.tmdb_id }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                    <div class="flex justify-around">
                                        <router-link
                                            class="block px-3 py-2 rounded-md text-base font-medium bg-green-50 text-blue-900 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700"
                                            :to="{ name: 'AdminSeason', params:{ serieId: serie.id }}">Seasons
                                        </router-link>
                                        <button @click.prevent="editSerie(serie)"
                                                class="text-green-600 hover:text-green-900">Edit
                                        </button>
                                        <button @click.prevent="destroySerie(serie)"
                                                class="text-red-600 hover:text-red-900 ml-3">Delete
                                        </button>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div class="mt-4">
                        <pagination
                            ...
                            @prev="getSeries(pagination.prevPageUrl)"
                            @next="getSeries(pagination.nextPageUrl)"
                        ></pagination>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        getSeries(url = '/api/admin/admin-series') {
            const _this = this;
            axios.get(url, {
                params: {
                    search: _this.search,
        ...
        createSerie() {
            ...
            } else if (this.serie.poster_path) {
                this.poster_url = 'https://image.tmdb.org/t/p/w185' + _this.serie.poster_path;
            }
            axios.post('/api/admin/admin-series', {
                'tmdb_id': _this.serie.tmdb_id,
                'name': _this.serie.name,
                'poster_path': _this.poster_url,
                'created_year': _this.serie.created_year
            })
                .then(res => {
                    this.$notify({
                        group: 'foo',
        ...
        updateSerie(serie) {
            ...
            axios.post('/api/admin/admin-series/' + serie.slug, {
                ...
                'name': serie.name,
                'poster_path': _this.poster_url,
                'created_year': serie.created_year,
                _method: "PUT"
            })
                .then(res => {
                    _this.getSeries();
        ...
        generateSerie() {
            ...
            this.isEdit = false;
            const _this = this;
            const url = `https://api.themoviedb.org/3/tv/${this.tmdb_id_g}?api_key=8a11aac3fb4ef5f1f9607ee7e0329793&language=en-US`;
            axios.get(url, {withCredentials: false})
                .then(res => {
                    ...
                    _this.serie.created_year = res.data.first_air_date;
                }).catch(error => {
            })
        },
        clear() {
