     ...
     <x-front.card>
        <x-slot name="image">
            <div class="aspect-w-2 aspect-h-3">
                ...
                <a href="{{ $result->slug }}" class="hover:scale-110 text-white opacity-0 transform translate-y-3 group-hover:translate-y-0 group-hover:opacity-100 transition">
          ...
          <div class="">
              <span class="m-1 p-1 text-main-rose">{{ $result->type }}</span>
              <a href="{{ $result->slug  }}">
                  <h3 class="text-white font-bold group-hover:text-blue-300 text-sm">{{ $result->title }}</h3>
              </a>
          </div>
      </x-front.card>      
    @endforeach
